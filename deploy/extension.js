/* TimeBlock Organizer v1.2.0 | MIT | generated from src/ */
...
function createLegacyRuntime({ extensionAPI }) {
  ...
  function childrenFromPull(data) {
    return (data?.[":block/children"] || []).map((c) => ({
      uid: c[":block/uid"],
  ...
  async function getDirectChildrenAsync(parentUid) {
    try {
      ...
      const data = typeof asyncPull === "function" ? await asyncPull(CHILDREN_PULL, [":block/uid", parentUid]) : window.roamAlphaAPI.data.pull(CHILDREN_PULL, [":block/uid", parentUid]);
      return childrenFromPull(data);
    } catch (e) {
  ...
  function conflictIgnoreTags() {
    return Array.from(new Set(String(state.settings.conflictIgnoreMarkers || "").split(",").map((s) => s.trim()).filter(Boolean)));
  }
  ...
  async function syncConflictStatus(pageUid, conflicts, pageChildren) {
    ...
    if (!state.settings.persistentConflictOutput || conflicts.length === 0) {
      for (const status of statuses) {
        ...
        try {
          ...
          log("warn", `delete status block ${status.uid} failed`, e?.message || e);
        }
      }
      return { writes };
    }
    ...
    for (const extra of statuses.slice(1)) {
      ...
      try {
        ...
        log("warn", `delete duplicate status ${extra.uid} failed`, e?.message || e);
      }
    }
    if (!canonical) {
      ...
      try {
        ...
        log("warn", "update status header failed", e?.message || e);
      }
    }
    const existing = await getDirectChildrenAsync(canonical.uid);
    if (state.disposed || !state.settings.enabled) return { writes };
    const sameLines = existing.length === lines.length && existing.every((child, index) => child.string === lines[index]);
    ...
    for (const child of existing) {
      ...
      try {
        ...
        log("warn", `delete conflict line ${child.uid} failed`, e?.message || e);
      }
    }
    for (let order = 0; order < lines.length; order++) {
      ...
      try {
        ...
        log("warn", "create conflict line failed", e?.message || e);
      }
    }
    return { writes };
  }
  ...
  async function applyDesiredOrder(pageUid, tbUid, desired, pageLevelMisplaced) {
    ...
    const exactSet = workingUids.length === desiredUids.length && desiredUids.every((uid) => currentSet.has(uid));
    if (!exactSet) {
    ...
    if (typeof api.reorderBlocks === "function") {
      try {
        ...
        const stillExact = fallbackUids.length === desiredUids.length && desiredUids.every((uid) => fallbackSet.has(uid));
  ...
  async function reconcileTimeBlock(pageUid, reason = "watch") {
    ...
    if (state.disposed || !state.settings.enabled) return;
    const { desired, pageLevelMisplaced, currentTbChildren } = computeDesiredOrderFromChildren(pageChildren, tbChildren, tbUid);
    ...
    if (state.disposed || !state.settings.enabled) return;
    const completedRanges = desired.filter(
      (item) => isTimePrefixed(item.string) && !isActiveSession(item.string)
    );
    const conflicts = state.settings.persistentConflictOutput ? detectOverlaps(completedRanges) : [];
  ...
  function timeBlockWatchFingerprint(pull) {
    ...
    return childrenFromWatchPull(pull).map((item) => `${item.uid}:${item.order}:${watchSortKey(item)}:${ignoreTags.some((tag) => item.string.includes(tag)) ? 1 : 0}`).join("|");
  }
  ...
  function addOwnedPullWatch(uid, callback, label) {
    ...
    return () => {
      ...
      ready.then((registered) => {
        ...
        try {
          return Promise.resolve(data.removePullWatch(WATCH_PULL, entity, guarded)).catch(
  ...
  function attachVisibilityListener() {
    ...
    const handler = () => {
      ...
      recoverToday("visibility-recovery").catch(
        (e) => log("warn", "visibility recovery failed", e?.message || e)
      );
    };
  ...
  function registerCommands() {
    ...
    add("TimeBlock Organizer: show conflicts on current page", async () => {
      ...
      const todos = finalChildren.filter((c) => isTimePrefixed(c.string) && !isActiveSession(c.string)).sort((a, b) => {
    ...
    add("TimeBlock Organizer: show stats (current settings)", () => {
      ...
      const lines = [
        ...
        `  Recovery: today only every ${RUNTIME.recoveryIntervalMs / 6e4}min while visible`,
        ``,
        `Watched pages:`,
  ...
  function activateMonitoring() {
    ...
    state.recoveryTimer = setInterval(() => {
      recoverToday("hourly-recovery").catch(
        (e) => log("warn", "hourly recovery failed", e?.message || e)
      );
    ...
    try {
      ...
      if (stored && typeof stored === "object") {
        for (const key of retainedKeys) {
          if (stored[key] !== void 0) found[key] = stored[key];
        }
      }
    } catch (e) {
      log("warn", "legacy local settings read failed", e?.message || e);
    }
    try {
      ...
      for (const [graphKey, key, type] of specs) {
        ...
        found[key] = type === "bool" ? ["true", "yes", "on", "1", "y"].includes(String(raw).trim().toLowerCase()) : String(raw).trim();
      }
    } catch (e) {
      log("warn", "legacy graph settings read failed", e?.message || e);
    }
    return found;
  }
  function applySettings(settings) {
    for (const key of Object.keys(DEFAULTS)) {
      if (settings?.[key] !== void 0) state.settings[key] = settings[key];
    }
  }
  function setSetting(key, value) {
    ...
    if (key === "persistentConflictOutput") {
      recoverToday("settings-change").catch(
        (e) => log("warn", "settings recovery failed", e?.message || e)
      );
    }
  }
  function start() {
...
// src/extension.js
var VERSION = "1.2.0";
var SETTINGS_MIGRATION_VERSION = 2;
var SETTINGS_MIGRATION_KEY = "settings-migration-version";
var LEGACY_SNAPSHOT_KEY = "settings";
var SETTING_SPECS = [
  ...
  { id: "active-session-signature", key: "activeSessionSignature", defaultValue: "{{\u21E5\u{1F55E}:SmartBlock:Elapsed time}}" },
  { id: "smartblock-button-signature", key: "smartblockButtonSignature", defaultValue: "{{\u{1F557}\u21A6:SmartBlock:Double timestamp buttons2}}" },
...
async function migrateSettings({ extensionAPI, runtime }) {
  ...
  if (marker !== SETTINGS_MIGRATION_VERSION && extensionAPI.settings.canSet !== false) {
    ...
    try {
      localStorage.removeItem("timeblock-organizer:settings");
    } catch {
    }
  }
