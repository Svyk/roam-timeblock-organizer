  ...
  Object.assign(target, {
    roamAlphaAPI: {
      ...
      data: {
        ...
        block: { create: async (...args) => calls.push(["block:create", ...args]), update: async (...args) => calls.push(["block:update", ...args]), delete: async (...args) => calls.push(["block:delete", ...args]), move: async (...args) => calls.push(["block:move", ...args]) },
      },
      util: { generateUID: (() => { let id = 0; return () => `test${++id}`; })() },
      ui: { commandPalette, mainWindow: { getOpenPageOrBlockUid: () => null }, rightSidebar: { addWindow: async () => null } },
    },
    alert: () => {}, confirm: () => false,
  });
